...
1. 固化 `SandboxTemplate` 契约：
   ...
   - Claude 运行时环境变量
2. 验证 SDK `port-forward` 模式与 `/agent` 桥接调用行为。
...
3. sandbox 通信统一经过 Go SDK/HTTP，而不是 Redis ingress。
