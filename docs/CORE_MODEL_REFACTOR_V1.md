...
> 状态：当前模型。本文只记录已确认的 core model，不包含 clawman 内部 sandbox 模块重设计。
...
Room 配置允许更新 `display_name`、`outbound_alias`、`prompt`，以及该 Room 默认 Agent Session 的 `enabled`、`trigger_policy`。Adapter 注册入口先按 `(tenant_id, channel, channel_room_id)` 查询 Room；如果 Room 已存在，直接返回现有 Room；如果不存在，才注册新 Room。`channel`、`channel_room_id`、`channel_room_type` 是 Room 的外部身份，不允许通过更新改变。

### Room Prompt

`Room Prompt` 是 Room 自身的行为设定，用于描述该房间的长期语气、边界、群规和场域信息。它存储在 `rooms.prompt`，不是独立实体，也不改变“每个 Room 只有一个长期 Agent Session”的模型。

Runner prompt 先注入固定 runner 规则，再注入 Room Prompt，随后注入被 `@` 命中的 run-scoped Subagent prompts、Memory Search 结果和当前 Message window。
...
`Subagent` 是一次 Agent Run 内部临时使用的 Agent 定义。当前实现会按本次 Message 中的 `@key` / `@display_name` 选择 enabled Agents，并把这些定义作为 run-scoped Subagents 注入 Codex prompt。Subagent 可以通过本次 Agent Run 的 memory search capability 读取 Room Memory，但不拥有 Room 身份、进度游标或 Room Memory 写入权。
...
`pending_trigger_message_id` 是该 Agent Session 当前需要追上的最新触发边界。入站 Message 命中该 Agent Session 的 Trigger Policy 时，更新该字段。Trigger Policy 同时支持立即触发规则（`mode`、`mentions`、`keywords`、`direct_default`、`ignored_senders`）和可选 `batch` 审阅触发；batch 不代表一定回复，只代表在高流量 Room 中按累计消息和时间窗口触发一次 Agent Run，由 Room Prompt 和上下文决定是否输出。
...
`agent_id` 为空表示默认 orchestrator 产生的写入；有值时表示某个 run-scoped Agent 定义参与了本次写入决策。Agent Run Result 中的 Memory Write Proposals 不直接修改 Room Memory，而是先写入 `memory_write_jobs`。后台 worker 异步应用 pending jobs，并做有限重试。
...
为避免初始部署没有可登录 client，第一版提供一个配置内建 admin client。默认 `client_id` 为 `admin`，`client_secret` 读取 `CLAWMAN_ADMIN_SECRET`；若未配置 secret，则禁用内建 admin client 和 `/admin/api/*`。该内建 client 不写入 `api_clients`，避免启动时修改数据库；`api_clients` 仅用于后续显式创建的外部或管理 client。

### query indexes

第一版只补核心路径索引：Room Timeline 使用 `messages(room_id, id desc)`；Delivery polling 使用 `deliveries(status, id)`；Timeline 关联 Delivery 使用 `deliveries(room_id, source_message_to_id)`；Agent Scheduler 使用 `agent_sessions(pending_trigger_message_id, caught_up_message_id)`；Room Memory 使用 `memory_items(room_id, status, updated_at desc)`；Memory Worker 使用 `memory_write_jobs(status, id)`；Memory token 清理使用 `memory_capability_tokens(expires_at)`。
...
Rooms
  ...
  -> Agents
     - mutable Agent definitions
     - prompt, description, allowed_tools, enabled state
```
...
- update `display_name`, `outbound_alias`, `prompt`, and `trigger_policy`
